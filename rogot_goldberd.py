...
class RogotGoldberd(BaseComparisons):
    """Class to calculate the Rogot-Goldberd index.
    ...
    def rg_1sim_wdis(self):
        """Calculate the index with 1-sim-counters and with weighted denominator.

        Note
        ----
        w_a/(2 * w_a + w_b + w_c) + w_d/(2 * w_d + w_b + w_c)
        """
        numerator1 = self.w_a
        numerator2 = self.w_d
        denominator1 = 2 * self.w_a + self.total_w_dis
        denominator2 = 2 * self.w_d + self.total_w_dis
        self.RG_1sim_wdis = numerator1/denominator1 + numerator2/denominator2

    def rg_1sim_dis(self):
        """Calculate the index with 1-sim-counters and with unweighted denominator.

        Note
        ----
        w_a/(2 * a + b + c) + w_d/(2 * d + b + c)
        """
        numerator1 = self.w_a
        numerator2 = self.w_d
        denominator1 = 2 * self.a + self.total_dis
        denominator2 = 2 * self.d + self.total_dis
        self.RG_1sim_dis = numerator1/denominator1 + numerator2/denominator2
